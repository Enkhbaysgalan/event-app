...
export default function CreateEventPage() {
  ...
  const validate = () => {
    ...
    if (!imageFile) e.image = "Please upload an event image";
    return e;
  };
    ...
    <div className="min-h-screen bg-[#0c0c12] text-white pb-32">
      ...
      <div className="px-5 flex flex-col gap-5">
          ...
          >
            {imagePreview ? (
              ...
              <>
                ...
                <p className="text-[10px] text-gray-700 font-mono">JPG, PNG, WEBP</p>
              </>
            )}
            {imagePreview && (
        ...
        <Field label="Venue Name" icon={MapPin} error={errors.location}>
          <TextInput value={form.location} onChange={set("location")} placeholder="e.g. Sky Lounge, Ulaanbaatar" />
        </Field>

        <Field label="Address / Detail">
          <TextInput value={form.locationDetail} onChange={set("locationDetail")} placeholder="e.g. 4th floor, Blue Sky Tower" />
        </Field>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50">
        <div className="max-w-lg mx-auto px-5 pb-6 pt-4 bg-gradient-to-t from-[#0c0c12] via-[#0c0c12] to-transparent">
          <button
            ...
            disabled={submitting}
            className="w-full h-[60px] bg-primary font-display font-black text-[15px] uppercase tracking-widest text-white active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {submitting ? (
              <>
